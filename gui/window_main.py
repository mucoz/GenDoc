...
from core.aux_classes import File
from config.logger import Logger
from core.app_state import AppState
from gui.window_recorder import RecorderWindow
...
class MainWindow:
    def __init__(self):
        self.config_parser = AppState.config_parser
        self.keyboard_listener = AppState.keyboard_listener
        self.mouse_listener = AppState.mouse_listener
    ...
    def create_elements(self):
        ...
        # get language manager from AppState
        self.lm = AppState.language_manager
        # frame for buttons
        self.frame_buttons = ttk.Labelframe(self.window, text=self.lm.controls, padding=2, borderwidth=1, relief="solid", bootstyle="info")
        self.frame_buttons.grid(row=0, column=0, sticky="nswe", padx=15, pady=15)
        # frame for directory entry and choose folder button
        self.frame_directory = ttk.Labelframe(self.window, text=self.lm.saved_docs, padding=2, borderwidth=1, relief="solid", bootstyle="info")
        ...
        # frame for list view
        self.frame_listbox = ttk.Labelframe(self.window, text=self.lm.files, padding=2, borderwidth=1, relief="solid", bootstyle="info")
        self.frame_listbox.grid(row=2, column=0, sticky="nswe", padx=15, pady=5)
        # frame for footer
        self.frame_footer = ttk.Labelframe(self.window, text=self.lm.info, padding=2, borderwidth=1, relief="solid", bootstyle="info")
        ...
        self.button_record = ttk.Button(self.frame_buttons,
                                   text=self.lm.new_doc,
                                   image=self.icon_record,
                                   compound="left",
                                   style="Large.TButton",
                                   command=self.show_recorder_window) # command=self.keyboard_listener.start
    ...
    def show_recorder_window(self):
        if self.recorder_window is None or not self.recorder_window.window_exists():
            self.recorder_window = RecorderWindow(self)
            # self.recorder_window.display()
            self.button_record["state"] = "disabled"
            self.window.iconify()

    def enable_start_recording_button(self):
        logger.info("Record button is being activated...")
        self.button_record["state"] = "normal"
        logger.info("Main window is being activated...")
    ...
    def open_file(self, file_path):
        ...
        if os.path.exists(file_path):
            ...
            utils.show_message("Warning", f"File not found: {file_path}", 16)
